...
class CNNEmbeddings(object):
    def __init__(self, sequence_length, num_classes, vocab_size, embedding_size,
                 filter_sizes, num_filters, l2_reg_lambda, non_static):
        ...
        with tf.name_scope('embedding'):
            if not non_static:
                self.W = tf.constant(embedding_size, name='W')
            else:
        ...
        with tf.name_scope('output'):
            ...
            self.softmax = tf.nn.softmax(logits=self.scores,name='softmax')
